...
-- Soldaki yada önce yazılanları temel olarak alıp yazarız sağdaki tablodan ise eşleşenleri getirir.
...
-- ürünlere ulaşırız. order_itemleri NULL ise bu ürünler product da var ama sipariş almamış demektir.
SELECT A.product_id, A.product_name, b.product_id  -- burada B.order_id de kullanabilirim. çünkü diğer tabloda 
FROM product.product A									-- karşılığı yok ise o sütun da null olarak gözükür.
...
-- bunun, join yapmadan yani daha az maliyetli bir çözümü de var:
SELECT COUNT(DISTINCT(staff_id))
FROM sale.orders;
-- siparişler (orders) tablosuna giderim. oradaki çalışan numaralarını (staff_id) distinct olarak saydırırım.
