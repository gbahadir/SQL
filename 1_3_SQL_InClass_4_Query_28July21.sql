...
WHERE model_year = 2017
ORDER BY 1
-- 3, 4 ve 5 brand_id si olan markalarýn sadece 2016 yýlý üretimleri olduðunu gördüm.
-- çünkü diðerleri 2017 de de üretildiði için EXCEPT ile çýkarýldý.
...
WHERE NOT EXISTS (
					...
					and X.state = D.state  -- dýþ query deki customers tablosu ile burdakini de join ediyor gibi..
					) 
...
AND		C.customer_id = D.customer_id
AND		A.product_name = 'Trek Remedy 9.8 - 2017'
-- Alttaki sorguda Trek Remedy 'in satýn alýndýðý state leri getirdiðine göre. 
-- EXCEPT ile kullandýðýmnýzda Trek Remedy 'in satýn alýnmadýðý state i bulmuþ olduk.
-- ( üstteki sorguda customer larýn state'lerinin tamamýný getiriyor.
